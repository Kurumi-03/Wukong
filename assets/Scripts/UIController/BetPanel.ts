...
export class BetPanel extends Component {
    ...
    private CreateIcon(x: number, y: number, clearNum: number, isMove: boolean = true,) {
        // 创建图标
        const newfruit = instantiate(this.iconPrefab[this.grid[x][y]]);
    ...
    private Clear(results: RepeatResult[]) {
        ...
        for (let i = 0; i < results.length; i++) {
            ...
            for (let i = 0; i < result.positions.length; i++) {
                ...
                // console.log(`需要消除的颜色为 ${fruit.name} ，所在位置x为 ${result.positions[i].x} 所在位置y为 ${result.positions[i].y}`);
                ...
                temp.getComponent(Icon)?.ClearEffect(index, () => {
                    if (isClear == false) {
                        isClear = true;
                        this.Drop();
                        EventManager.Send("UpdateWinScore", DataManager.Instance(DataManager).winScore[this.gameCount]);
                    }
                });
            }
        }
